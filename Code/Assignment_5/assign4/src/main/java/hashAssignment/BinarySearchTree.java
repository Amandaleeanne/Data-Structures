...
{
    private int treeSize = 0; //for easier management of the hash table
    /**
    ...
    /**
     * Removes the largest node in the tree and returns it
     * @return
     */
    public Node removeAndShare()
    {
        RemovalResult result = removeLargest(root);
        root = result.tree;
        treeSize--;
        return result.node;
    }
    ...
    public LinkedList treeAsLinkedList()
    {
            LinkedList treeAsList = new LinkedList<>();
            asList(root, treeAsList);
            return treeAsList;

    }
    private  void asList(Node node, LinkedList list) {
        if (node != null) {
